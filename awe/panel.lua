...
awesome.connect_signal("evil::mail_main", function(evil)
	if evil ~= "nop" then
		...
		mail_main:set_markup(markup(beautiful.mauve, evil))
...
awesome.connect_signal("evil::mail_ims", function(evil)
	if evil ~= "nop" then
		mail_ims:set_markup(markup(beautiful.mauve, evil))
...
screen.connect_signal("request::desktop_decoration", function(s)
	...
	awful.tag({ "hello", "world", "3" }, s, awful.layout.suit.tile)
	...
	-- 		if self.bg ~= beautiful.bg_color_light then
	-- 			self.backup = self.bg
