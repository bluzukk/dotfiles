...
local LAT = util.read_line(LAT_FILE)
if not LAT then
	Show_Warning("Error when setting up weather widget: LAT MISSING")
	return -1
end

local LONG = util.read_line(LONG_FILE)
if not LONG then
	Show_Warning("Error when setting up weather widget: LONG MISSING")
	return -1
end

local API = util.read_line(API_KEY_FILE)
if not API then
	Show_Warning("Error when setting up weather widget: OWM API MISSING")
	return -1
end
...
local function worker(user_args)
		...
		update = function(self, forecast, timezone_offset)
			...
			for i, day in ipairs(forecast) do
				...
				local day_forecast = wibox.widget({
					...
					{
						markup = markup(
							beautiful.accent_alt_color,
							markup.bold(os.date("%a", tonumber(day.dt) + tonumber(timezone_offset)))
						),
						align = "center",
						font = font_name .. " 14",
						widget = wibox.widget.textbox,
					...
					},
					{
						{
							markup = markup(
								beautiful.main_color,
								gen_temperature_str(day.temp.day, "%.0f", false, units)
							),
							align = "center",
							font = font_name .. " 12",
							widget = wibox.widget.textbox,
	...
	--                     {
	--                         {
	--                             markup = markup(beautiful.main_color, gen_temperature_str(day.temp.day, '%.0f')),
	--                             align = 'center',
	--                             font = font_name .. ' 12',
	--                             widget = wibox.widget.textbox
	--                         },
	...
	--                             gen_temperature_str(day.temp.night, '%.0f')),
	--                             align = 'center',
	--                             font = font_name .. ' 12',
	--                             widget = wibox.widget.textbox
	--                         },
	...
	local current_weather_widget = wibox.widget({
		...
		update = function(self, weather)
			self:get_children_by_id("temp")[1]:set_markup(
				markup.fontfg(
					beautiful.font_name .. " 21",
					beautiful.accent_color,
					markup.bold(gen_temperature_str(weather.temp, " %.0f"))
				)
			)
			self:get_children_by_id("feels_like_temp")[1]:set_markup(
				markup(
					beautiful.accent_color_dark,
					"  " .. LCLE.feels_like .. gen_temperature_str(weather.feels_like, "%.0f")
				)
			)
			self:get_children_by_id("description")[1]
				:set_markup(markup(beautiful.accent_color, "  " .. beautiful.uwu_map[weather.weather[1].description]))
		...
		widget:setup({
			{
				...
				bg = beautiful.bg_dashboard_item,
